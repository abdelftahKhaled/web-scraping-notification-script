...
def flamecomics_website():
        ...
        if response.status_code == 200:
            ...
            for x in main_tag:
                ...
                if convert_time_to_seconds_long_word(time_publish) < 20000:
...
def hidden_file():
    ...
    subprocess.check_call(["attrib","+H",os.path.join(current_directory,'databas.db')])
    subprocess.check_call(["attrib","+H",os.path.join(current_directory,'iconn.png')])
...
def filter_articles_in_database_and_send():
    ...
    conn = sqlite3.connect('database.db')
    cur = conn.cursor()
    # SELECT id ,  website , title , chapter,link,created_at,is_send
    select_sql = '''
    SELECT website, GROUP_CONCAT(DISTINCT id),GROUP_CONCAT(DISTINCT title), GROUP_CONCAT(DISTINCT chapter) ,GROUP_CONCAT(DISTINCT link ) 
    FROM articles
    WHERE is_send = 0
    GROUP BY website;
    '''
    ...
    for  website,id, Titles,chapters,links in rows:
        ...
        for _id,tit,ch,lin in zip( id.split(','),Titles.split(','), chapters.split(','),links.split(',')):
                ...
                index=index+1
                cur.execute("UPDATE articles SET is_send = ? WHERE id = ?", (1, _id))
        final_content=start_content+content
        
        new_text_file(website=website ,content=final_content,couter_to_item=index)
        import time
        time.sleep(15)    
    conn.commit()
    conn.close()
