   ...
   """
    ...
    for  website,id, Titles,chapters,links in rows:
        ...
        if (website and id and Titles and chapters and links)==None:
         print(f'No now  articles at {_time}')
        else: 
    ...
    for website , content , couter_to_item,id in all_data:
        ...
        for c in id.split(','):
                # print(int(c))
                cursor.execute('UPDATE articles SET is_send = ? WHERE id =?; ',(1,c,))
                conn.commit()
                time.sleep(.1)
...
# if __name__ == "__main__":
    ...
    #                 try:
    #                     print(f'New search at {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
    #                     suryatoon_website()
    #                     hidden_file()
    #                     freakcomic_website()
    #                     night_scans_website()
